...
const AcademyAnalysisDashboard = () => {
  ...
  const getEvaluationStatus = (player: any) => {
    // If the mock data has evaluationDate, we can use that as fallback if there is no record in Redux
  ...
  const playersWithStatus = players.map(player => ({
    ...
    if (!a.status.expired && b.status.expired) return 1;
    // Both expired: sort by smaller remainingDays (most overdue)
    if (a.status.expired && b.status.expired) {
      return a.status.remainingDays - b.status.remainingDays;
    }
    // Both active: sort by smaller remainingDays (closest to expiration)
    return a.status.remainingDays - b.status.remainingDays;
  });
